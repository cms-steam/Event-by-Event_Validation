...
void makePlots_hlt_eventbyevent_comparison_twofiles( std::string referenceFile_="input1.root", std::string targetFile_="input2.root", std::string target_label_="Target", std::string outputSuffix_="_out" ) {

  TFile *ref_file = new TFile(referenceFile_.c_str());
  TFile *tgt_file = new TFile(targetFile_.c_str());
  ...
  TH1D* h_ref = (TH1D*)ref_file->Get("eventbyevent/HLT_Tgt");
  TH1D* h_tgt = (TH1D*)tgt_file->Get("eventbyevent/HLT_Tgt");
  TH1D* h_tgt_sep = (TH1D*)tgt_file->Get("eventbyevent/HLT_Tgt_sep");
  ...
  for( int iPath=0; iPath<h_tgt_sep->GetNbinsX(); iPath++ ){
    ...
    if( rel_diff>0.05 ) hlt_common_paths.push_back(name);
  }
  ...
  if( true ){
    ...
    for( int iPath=0; iPath<numCommonPaths; iPath++ ){
      ...
      TH1D* h_hlt_filt_ref = (TH1D*)ref_file->Get(pathName_ref.c_str());
      TH1D* h_hlt_filt_tgt = (TH1D*)tgt_file->Get(pathName_tgt.c_str());
